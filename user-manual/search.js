...
(function () {
  ...
  const searchIndex = [
    ...
    { page: 'Home', title: "What's New in v2.3.1", section: "What's New", text: 'PDF export Hindi Devanagari Bengali Gujarati Gurmukhi Kannada Malayalam Odia Sinhala Tamil Telugu Thai blank page missing text no glyphs font bundled Noto Sans subset Indic vowel sign reorder matra. Chinese Japanese Korean CJK kanji kana hangul Han downloaded on first use cached font CDN wide characters line breaks margin. Hebrew Arabic right to left RTL reversed backwards bidirectional bidi order Persian Urdu letter shapes joined presentation forms not mirrored margins. Word DOCX complex script font Courier Prime wrong typeface substitution Hindi in a Latin face ascii hAnsi cs eastAsia. Font failed to load warning named unsupported script Khmer Tibetan Lao Myanmar Syriac Ethiopic silent blank export print. Export print Android 12 older permission denied os error 13 no permission prompt save dialog did nothing PDF Final Draft Fountain Word. AV script two column cue timing column shot number start time duration runtime clock. Storyboard column frame aspect ratio 16:9 4:3 picture image per shot. Column width drag divider resize narrower wider AV columns. Screenplay elements inside AV cells Character Dialogue Parenthetical Action Shot Lyrics video audio column template decides. Export AV YAML xlsx csv spreadsheet plain text shot list import Visual Picture Vision Sound Narration VO Length Run Time. AV row controls touch phone tablet plus Row delete button long press Format AV Script menu Tab Cmd Enter. Line After Table Line Before Table exit leave escape get out write outside the table stuck trapped. Android storyboard frame picture not showing blank image picker photo did nothing. Image dropped pasted in AV row split the table second table empty row. PDF pagination page breaks match the editor wrong page count 33 35 impagination. Sign in fresh desktop install error sending request for url localhost port dead. Browser tab recovery unsaved changes overwritten by a second tab own slot. Download asset filename not ASCII screenshot narrow space server error. Custom template as default for new scripts template manager Script Format Preferences picker opens on it selected. Blank white window Linux AppImage Arch Fedora will not start empty grey nothing draws libwayland EGL. Mac App Store said up to date no update notice offered wrong version stores publish separately. Handwriting panel lost the cursor inserted in the wrong place keyboard label stale resize rewrap. Handwriting panel iPad floating stays open drag title bar tap behind resize see through ink colour. Selection handles iPad grabber widen selection stays visible highlight disappears. Print prints the pages you are looking at PDF page breaks match editor browser dialog Settings Printing. Print did nothing desktop app File Print menu silently. Printing from the File menu printed the menu. Paste as Fountain plain line split the line ellipsis leading markup. Blank line blank lines consecutive Enter element menu escape exit turn off switch Settings Editing. Element menu shortcut Alt Enter Option Return configurable custom keys record. Footnotes missing lost page ends MORE speech carried over split. Speech will not split break page orphan single line CONTD band too tall. Context menu submenu off screen edge element style revision color add to dictionary flip left scroll. ALL CAPS several lines merged selection formatting bold italic colour lost. One version number everywhere same across platforms App Store 1.8 0.26.3 2.0.0 unified matches. Paste Android clipboard Edit Paste as Fountain could not read the clipboard failed Samsung. Bold italic colour formatting preserved. Title page cleared emptied blank space gap left behind removed. Include Title Page moved File menu to Title Page editor dialog checkbox tick immediately. Title page Fade In fadein placeholder boilerplate TITLE Author Name Draft information never wrote imported. Final Draft fdx export Untitled Screenplay invented title page round trip. Written by twice credit line duplicated notes. Include title page Format Title Page dialog checkbox print PDF Word exclude omit leave out remembered. Print iPhone iPad share sheet AirPrint Android system print dialog Save as PDF. Panels status bar notes characters tags scene navigator close button. About Diagnostics full screen height. User manual in the app offline Help menu download read without connection. Search the manual pages titles context. Update remove downloaded manual storage. New logo icon mark app icon favicon small sizes. Help menu next to Tools three dots. Keyboard shortcuts moved to the manual. About compatibility table collapsed hidden. Header footer own dialog Format menu. Page number first page show on first page checkbox. Start numbering at synopsis front matter. Double click edit header footer in place. Word DOCX page numbers title page. Final Draft fdx header footer export. PDF export Cyrillic Russian Greek Armenian Georgian alphabet non-Latin garbled characters font. Export filename keeps title any language Untitled. Automatic backups iPad iPhone Android folder Files iCloud Drive Google Drive Dropbox. Multiple windows new window split screen. Open and save files in place fdx fountain fadein osf. Export Fade In fadein writer. Recover unsaved changes per window. File close back button touch targets. Start element on new page. Acts own page templates. Fonts in exports PDF Word Final Draft. Fade In import fadein osf. Paste as Fountain. Hard line breaks shift enter. Save reliability. Insert images title page scene navigator version diff DOCX import export script statistics collaboration.', url: 'index.html#whats-new' },
    ...
    { page: 'Writing Your Screenplay', title: 'Switching Elements', section: 'Switching', text: 'Keyboard shortcut Cmd 1 through 8. Toolbar dropdown element type. Format menu. Element menu. Auto-switching after enter scene heading to action character to dialogue.', url: 'writing-screenplay.html#switching-elements' },
    { page: 'Writing Your Screenplay', title: 'The Element Menu', section: 'Switching', text: 'Element menu opens at the cursor list of element types. Enter on a blank line opens it. Option Return Alt Enter shortcut opens it anywhere on a line with text type chooser. Change the shortcut Settings Editing click the box press the keys. Current type at the top.', url: 'writing-screenplay.html#element-menu' },
    { page: 'Writing Your Screenplay', title: 'Blank Lines', section: 'Switching', text: 'Blank line blank lines consecutive extra space. Enter again inserts the line nothing selected. Blank Line row at the foot of the menu tap phone tablet touch. Choose the element the line is already on top row. Escape then Enter. Keep typing any other key closes the menu. After a blank line the menu stops asking Enter inserts straight away. Turn off Offer the element menu when I press Enter on a blank line Settings Editing switch off disable.', url: 'writing-screenplay.html#blank-lines' },
    ...
    { page: 'Formatting', title: 'Your Default Template', section: 'Templates', text: 'Set custom template as default for new scripts. Make default button template manager Script Format Preferences. New screenplay format picker opens on my own template selected first ready press Enter. Other formats still one click. Only one format selected no picker at all. Change or clear the default. Remembered per device.', url: 'formatting.html#default-template' },
    ...
    { page: 'Title Page', title: 'Leaving the Title Page Out', section: 'Include In Output', text: 'Include title page when printing or exporting to PDF or Word checkbox. Leave out omit exclude skip title page from print PDF Word docx. Remembered per device not saved in file not per script collaboration. Takes effect immediately no Apply needed. Final Draft Fountain Fade In always carry title page. Scene content starts page 1.', url: 'title-page.html#include-in-output' },
    ...
    { page: 'Writing Your Screenplay', title: 'Handwriting with an Apple Pencil', section: 'Handwriting', text: 'Apple Pencil Scribble iPad handwriting input box. Edit menu Handwriting three-finger context menu. Write with pencil insert at cursor. Replaces selection. Floating panel stays open drag by title bar move insertion point tap script preview clear close. Scribble overwrites deletes existing text middle of screenplay cramped small writing area. Safe large writing box. Resize panel corner drag bigger smaller remembered. Buttons become icons narrow compact. Transparency opacity see through panel writing area settings gear. Ink colour color blue red green purple legible. Script does not scroll move on insert stays put.', url: 'writing-screenplay.html#handwriting' },
    { page: 'Writing Your Screenplay', title: 'Adjusting a selection on iPad', section: 'Handwriting', text: 'iPad iPadOS text selection handles grabbers drag either end widen narrow selection. Press and hold select word. Drag past the other reverses. Highlight stays visible while using a menu or when the keyboard appears and disappears.', url: 'writing-screenplay.html#ipad-selection' },
