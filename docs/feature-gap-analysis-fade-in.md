...
| 16a | **Per-element Space Before** | Element Styles expose "the spacing before the paragraph" per element, so a writer can match another application's layout. | **Implemented (v0.24).** Space Before is editable per element in Format → Formatting Template, in blank lines. It previously existed as "Margin Top (pt)" but reached CSS only — page breaks, PDF and DOCX all used a hardcoded map, so changing it moved the gap on screen without moving the page. All four now read the template (`utils/elementSpacing.ts`). Scene Heading's default is 2 lines, matching Final Draft; scripts written under the old 1-line default are asked once whether to adopt it. |
